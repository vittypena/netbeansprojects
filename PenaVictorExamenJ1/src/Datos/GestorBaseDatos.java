...
public class GestorBaseDatos {
    ...
    private static final String SQL_INSERT = "INSERT INTO materias_primas(simbolo, nombre, p_venta, p_compra) VALUES(?, ?, ?, ?)";//Las interrogaciones indican que va a ser el parametro nombre, apellido, email y telefono en ese orden
...
//    }
    ...
    public int insertar(materia articulo) throws SQLException ,Excepciones{
        Connection conn = null;
        PreparedStatement stmt = null;
        int registros = 0;
        try {
            ...
            stmt.setString(1, articulo.getSimbolo());
            stmt.setString(2, articulo.getNombre());
            stmt.setDouble(3, articulo.getP_venta());
            stmt.setDouble(4, articulo.getP_compra());
            registros = stmt.executeUpdate();//Regresa el numero de registros afectados
        } catch (Excepciones ex) {
            throw new Excepciones("Error al insertar la materia en la bd materias_primas");
        } finally {
            try {
                Conexion.closeConnection(stmt);
